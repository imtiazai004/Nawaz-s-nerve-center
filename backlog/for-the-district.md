...
| # | What is needed | Status | Meanwhile | Until then |
|---|---|---|---|---|
| **R-05** | **WhatsApp, SMS and voice** (P-11, ADR-0012): a Meta business account with **pre-approved templates**, an SMS gateway, a telephony provider, and a **GSM modem + SIM** for the DC server | `OPEN` | The in-app inbox, which needs no provider. Adapters are built and tested against fakes | Notifications reach the app and nothing else. **This is the longest lead time in the project** — Meta must approve the alert wording before a single alert can be sent |
| **R-06** | **A Google Cloud Storage bucket and service account** (P-12, ADR-0011) | `OPEN` | Verified local dumps | Backups exist and never leave the building. Fire, flood or theft takes the record with them |
| **R-07** | **The AC Headquarter standby machine, and a network link between the two offices** (M0-54, ADR-0011) | `OPEN` | A single node in the DC office | A dead DC machine stops the district until it is repaired |
...
|---|---|---|
| **D-01** | Should a department be able to create posts **above station tier**? | **No.** The read model widens at tehsil, so a department able to place its own tehsil post would be granting itself sight of every incident in Bannu. The two offices can place any tier |

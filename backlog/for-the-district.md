...
| **R-15** | **The facts about Bannu that do not change on a Tuesday** — number of tehsils, union councils, population, area | `OPEN` | **The four rows exist with labels and no values** (migration 0017). The two offices type them at **Administration → Status**; deliberately free text, because "1,167,892" and "1,227 km²" are how a district writes them | The dashboard's district-facts panel renders blanks. A status board missing its own population has a job it is not doing, and these are the numbers every report and briefing opens with |
| **R-16** | **Which point should the weather be taken at?** The DC office, Bannu city, or a tehsil that floods first | `OPEN` | Bannu city's published coordinates — `32.9889, 70.6056` — as a **default, not a finding**. `WEATHER_LAT` / `WEATHER_LON` override without a release | A reading for the wrong point is worse than none during a flood: it is confidently wrong about the one thing the panel exists to warn about. **Was raised as "R-13" and renumbered 2026-08-03** — that number was already Rescue's response actions |
| **R-17** | **What does each department actually submit upward, and on what form?** A monthly return, a incident register, a format somebody's office already expects | `OPEN` | **A general spreadsheet export exists** — `Export incidents` gives every incident the caller may see, as CSV, opening correctly in Excel. It is not shaped like anybody's form | This is the **direct cost of the district running independently** (Q-01, Q-02). Export is what was promised instead of integration, and a generic file still leaves somebody retyping into their own format. **Double entry is the top adoption risk**: a department that retypes will eventually stop using the system it retypes *from*. One example of each real form is enough to close most of this |

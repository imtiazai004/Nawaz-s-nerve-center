...
| # | What is needed | Status | Meanwhile | Until then |
|---|---|---|---|---|
| **R-05** | **WhatsApp, SMS and voice** (P-11, ADR-0012): a Meta business account with **pre-approved templates**, an SMS gateway, a telephony provider, and a **GSM modem + SIM** for the DC server | `OPEN` | Everything but the accounts is done. The ladder is built and configurable, all five adapters are written and tested against fakes, and **[`docs/09-notification-templates.md`](../docs/09-notification-templates.md) has the four WhatsApp templates drafted for you to correct and submit** | Alerts reach the in-app inbox and nothing else — an officer not looking at the app is not told. **The longest lead time in the project**: Meta must approve the wording before a single alert can be sent, and that clock does not start until somebody submits it. The console says so on **Administration → Alerts** |

...
| M1a-09 | Collapse `Tier` to two rungs | `DONE` | ADR-0010 | Migration 0010. **Not cosmetic in the end:** the contact list has no tier column, so the loader had defaulted all 83 of the district's posts to `district` — and `evaluateRead` widened at tehsil, so every department could read every other department's incidents. Tier is now derived from `is_administration` by a database trigger and cannot drift |
| M1a-10 | The roster: people and numbers per department, **editable by the department itself** | `DONE` | M1a-03 | Who holds which post, right now. Notifications land on a post (ADR-0004) and the roster is what turns that into a person to reach. Scoped two ways from one screen: the two offices see all 79, a department sees itself. **Routing signals and SLA deadlines stay with the administration** — a department able to edit its own routing could quietly stop receiving night calls. Migration 0009, `db/rosterStore.ts`, `api/roster.ts`, `web/src/roster.ts`. 28 API + 11 browser tests |
...
| M1-06 | The post-incident report, folded from the log | `DONE` | M1-05 | `domain/report.ts` + `GET /incidents/:id/report[?format=text]`. Nothing typed: if an operator retypes what the system knows, the retyped version becomes a second account free to disagree with the first. Every duration measured from `occurredAt`, so an hour on a handset with no signal reads as an hour and not as speed. **Ends with what the record does not contain** — the section a hand-written report always omits, and the one a review most needs. Plain text, because Q-02 made export the point and nobody upstream should need this software installed. 19 tests |
| M1-07 | **The M1 gate** | `DONE (my half)` | M1-01…06 | `src/__tests__/m1gate.e2e.test.ts`. One emergency from a handset to a post-incident report, in a browser, against a real database, nothing stubbed. Rapid intake re-measured: **1.8s against a 15s budget** at 4× CPU throttle. Shell 123 KB against a 160 KB budget, asserted so growth is noticed here rather than in Domel. **The operator half is R-12 and I cannot do it** — the milestone says *a real Rescue operator, no developer present* |
